...
export const FrameworkProvider = ({ children }) => {
  ...
  // Fetch available frameworks from API on mount
  // FrameworkContext.jsx — no user dependency needed
  useEffect(() => {
    fetchFrameworks()
      .then((frameworks) => {
  ...
  }, []); // no user dependency — sessionStorage is read inside the service
  ...
  const moduleVisibility = computeModuleVisibility(selectedFrameworks, availableFrameworks);
