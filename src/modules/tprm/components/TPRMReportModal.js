...
// //   const [questionDecisions, setQuestionDecisions] = useState({}); // questionId → { status, remark }
...
// //     // Validate all questions have a decision
// //     const undecided = responses.filter(r => !questionDecisions[r.questionId]?.status);
...
// //         status:     questionDecisions[r.questionId]?.status || "ACCEPTED",
// //         remark:     questionDecisions[r.questionId]?.remark || "",
// //       }));
// //       await tprmService.submitReview(selected.id, reviewItems, user?.name);
...
// //     try {
// //       if (type === "approve") {
// //         await tprmService.approveQuestionnaire(selected.id, comment, user?.name);
// //       } else {
// //         await tprmService.rejectQuestionnaire(selected.id, comment, user?.name);
// //       }
// //       setDecision(null);
// //       setComment("");
// //       const updated = await tprmService.getQuestionnaire(selected.id);
...
// //   const canFinalDecision = (status) => ["UNDER_REVIEW", "RESUBMITTED"].includes(status);
...
// //                           <span><span className="font-medium">Due:</span> {q.dueDate}</span>
// //                         </div>
// //                       </div>
// //                       {reviewable && (
...
// //                     </span>
// //                   </span>
// //                   <span><span className="font-semibold">Due:</span> {selected.dueDate}</span>
// //                   {responses.length > 0 && (
...
// //                 {/* Change 2: Single Finalize button */}
// //                 {canFinalDecision(selected.status) && (
// //                   <button onClick={() => setDecision("finalize")}
// //                     className="flex items-center gap-1.5 px-4 py-2 bg-gradient-to-r from-indigo-500 to-indigo-600 text-white text-xs font-semibold rounded-xl hover:from-indigo-600 hover:to-indigo-700 transition-all shadow">
// //                     <CheckCircle size={12} /> Finalize Assessment
// //                   </button>
// //                 )}
// //               </div>
...
// //               {/* Change 2: Unified Finalize modal */}
// //               {decision === "finalize" && (
...
// //                                   {/* Existing decision badge (after review submitted) */}
// //                                   {r.questionStatus && (
...
// //                                     {r.availability ? (
// //                                       <span className={`text-xs font-semibold px-2 py-0.5 rounded-lg ${avCfg.cls}`}>
// //                                         {r.availability}
// //                                       </span>
...
// //                                     </>
// //                                   ) : (
// //                                     // Read-only view after review submitted
...
// //               {/* Submit Review button — sticky at bottom */}
...
//     // Step 1 — Submit per-question review → moves to UNDER_REVIEW
//     await tprmService.submitReview(
//       selected.id, 
//       reviewItems, 
//       user?.name, 
//       user?.email  // ← adminEmail for UNDER_REVIEW notification to vendor
//     );

//     // Step 2 — If all accepted → auto approve
//     const hasRejected = reviewItems.some(r => r.status === "REJECTED");
//     if (!hasRejected) {
//       await tprmService.approveQuestionnaire(
//         selected.id, 
//         "All questions accepted", 
//         user?.name,
//         user?.email  // ← adminEmail for APPROVED notification to vendor
//       );
//     }
//     // If hasRejected → vendor gets UNDER_REVIEW email, no approve needed
...
//       );
//     } else {
//       await tprmService.rejectQuestionnaire(
//         selected.id, 
//         comment, 
//         user?.name,
//         user?.email  // ← add this
...
//                 {/* Auto-approved or sent back via Submit Review — no manual finalize needed */}
//               </div>
...
//                                 {/* Admin per-question decision */}
//                                 {/* On RESUBMITTED: grey out already-accepted, only allow review of re-answered */}
//                                 {reviewable && r.questionStatus === "ACCEPTED" ? (
//                                   // Already accepted in previous round — greyed out, locked
...
//                                     </div>
//                                     {r.questionRemark && (
//                                       <p className="text-xs text-slate-500 mt-1 ml-5">{r.questionRemark}</p>
//                                     )}
//                                   </div>
//                                 ) : (
//                                 <div className={`ml-7 rounded-xl p-3 border ${
