  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        ...
        setError(err.response?.data?.message || err.message || "Failed to load compliance history.");
      } finally {
        setLoading(false);
      }
    };
      ...
      <Paper style={{ padding: 30, marginTop: 30, borderRadius: 12 }}>
        ...
        {loading ? (
          <Box display="flex" justifyContent="center" alignItems="center" height={400} style={{ gap: 16, flexDirection: "column" }}>
            ...
            <Typography style={{ color: "#64748b", fontSize: 15 }}>Loading compliance history...</Typography>
          </Box>
        ) : error ? (
