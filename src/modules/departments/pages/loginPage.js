...
const LoginPage = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const domainLookupRes = await axios.get(
        `${process.env.REACT_APP_SP}/compliance-brain/api/tenant-lookup/${tenantId}/domain`,
      );
  ...
  const sendOtp = async () => {
    ...
    try {
      await axios.post(
        `${process.env.REACT_APP_CFTB}/user-service/api/users/forgot-password`,
        { email: forgotEmail },
    ...
    while (attempt < maxRetries && !success) {
      try {
        const res = await axios.post(
          `${process.env.REACT_APP_CFTB}/user-service/api/users/verify-otp`,
