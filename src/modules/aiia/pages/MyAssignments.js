...
import {
  ...
  Clock, ChevronRight, RefreshCw, PartyPopper,
} from 'lucide-react';
...
function MyAssignments({ onSelectAssignment }) {
  ...
  const getStatusMeta = (status) => {
    switch (status) {
      case 'DRAFT':
        return { label: 'Action Required', color: '#f59e0b', bg: '#fef3c7', icon: <Clock size={13} /> };
      case 'SUBMITTED':
        return { label: 'Completed', color: '#10b981', bg: '#d1fae5', icon: <CheckCircle size={13} /> };
      case 'APPROVED':
        return { label: 'Approved', color: '#6366f1', bg: '#ede9fe', icon: <CheckCircle size={13} /> };
      case 'REJECTED':
        return { label: 'Rejected', color: '#ef4444', bg: '#fee2e2', icon: <AlertTriangle size={13} /> };
      default:
  ...
  return (
    <div className="assignments-page">
      <div className="assignments-header">
        ...
        <button className="refresh-btn" onClick={fetchAssignments} disabled={loading}>
      ...
      <div className="assignments-tabs">
        <button
          ...
          onClick={() => {
            ...
            captureActivity({ action: 'AIIA_TAB_NAVIGATION', item: 'Navigated to Pending Action AI Assessments', tab: 'pending' });
          }}
        >
          Pending Action
        ...
        <button
          ...
          onClick={() => {
            ...
            captureActivity({ action: 'AIIA_TAB_NAVIGATION', item: 'Navigated to Completed AI Assessments', tab: 'completed' });
          }}
        >
          Completed
          {completed.length > 0 && (
            <span className="tab-badge tab-badge-neutral">{completed.length}</span>
          )}
        </button>
      </div>
              ...
              >
                <div className="assignment-card-left">
                  <div
                    ...
                    style={isDone ? { background: '#d1fae5', color: '#10b981' } : {}}
                  >
                    {isDone ? <CheckCircle size={20} /> : <ClipboardList size={20} />}
                  </div>
                  <div className="assignment-info">
                    <h3 style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      {assignment.aiSystemName}
                      {assignment.status === 'SUBMITTED' && (
                        <PartyPopper size={15} style={{ color: '#10b981', flexShrink: 0 }} />
                      )}
                    </h3>
                    ...
                    <p className="assignment-objective">{assignment.businessObjective}</p>
                  </div>
                </div>
                <div className="assignment-card-right">
                  ...
                  <button
                    className="start-btn"
                    style={isDone ? { background: '#f1f5f9', color: '#64748b' } : {}}
                  >
