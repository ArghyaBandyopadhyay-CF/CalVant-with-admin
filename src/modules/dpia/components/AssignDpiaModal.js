...
export function AssignDpiaModal({ onClose, onSaved, riskOwners = [], departments = [] }) {
  ...
  useEffect(() => {
    const orgId = sessionUser.organization || sessionUser.organizationId || "";
    captureActivity({ action: "DPIA_PLAN_MODAL_OPENED", item: "Opened Plan DPIA Modal", url: window.location.pathname });
    dpiaApi
      .getAllAssessments(orgId)
              ...
              <button
                onClick={async () => {
                  try {
                    ...
                    setDpias(prev => [newDpia, ...prev]);
                    setSelected(newDpia);
                    captureActivity({ action: ACTIONS.CREATE, item: `DPIA · Created New DPIA Assessment: ${newDpia.id}`, url: window.location.pathname });
                    setStep(2);
                  } catch (err) {
                    console.error(err);
                  } finally {
