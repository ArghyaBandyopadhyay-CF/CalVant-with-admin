...
const OWNERSHIP_API =
  "https://api.calvant.com/control-ownership-service/api/control-ownership";
...
function deriveStatus(soaEntry, doc) {
  ...
  if (doc.nextApprovalDate && new Date(doc.nextApprovalDate) < new Date())
    return "rework";
  return "approved";
}
...
const FrameworkCell = ({ framework, mappings, colorMap }) => {
  ...
  return (
    ...
    >
      ...
      {mapped.map((fw) => (
        <FrameworkBadge key={fw} framework={fw} color={colorMap[fw]} />
      ))}
    </div>
  );
};
  ...
  return (
    ...
    >
      ...
      >
        <span>{label}</span>
        {isAsc ? (
          <ArrowUp size={13} style={{ color: "#667eea" }} />
        ) : isDesc ? (
          <ArrowDown size={13} style={{ color: "#667eea" }} />
        ) : (
...
// ── Name field helpers ─────────────────────────────────────────────────────
function parseDocName(storedName = "") {
  if (storedName.includes("||")) {
    return {
      compositeKey: storedName.split("||")[0].toLowerCase(),
      actualName: storedName.split("||")[1],
    };
  }
  return { compositeKey: null, actualName: storedName };
}

function buildCompositeKey(controlCode, docType, docName) {
  return `${controlCode}::${docType || docName}`.toLowerCase();
}
...
function AddTaskModal({ row, user, users, departments, onClose, onSuccess }) {
  ...
  const handleSave = async () => {
    if (
      ...
      setError(
        "Please fill all required fields (Department, Description, Start Date, End Date).",
      );
      return;
    }
          ...
          <div style={{ display: "grid", gap: 14 }}>
            <div>
              <label style={labelStyle}>
                Department <span style={{ color: "#c92a2a" }}>*</span>
              </label>
              <select
            ...
            <div>
              <label style={labelStyle}>
                Task Description <span style={{ color: "#c92a2a" }}>*</span>
              </label>
              <textarea
            ...
            >
              <div>
                <label style={labelStyle}>
                  Start Date <span style={{ color: "#c92a2a" }}>*</span>
                </label>
                <input
                  type="date"
              ...
              <div>
                <label style={labelStyle}>
                  End Date <span style={{ color: "#c92a2a" }}>*</span>
                </label>
                <input
                  type="date"
                  value={form.endDate}
...
const MLD = () => {
  ...
  const [userMap, setUserMap] = useState({}); // { [ownerId]: name }
  ...
  useEffect(() => {
    if (deleteModal.open) {
      captureActivity({
        ...
        item: [
          {
            detail: "Documentation · Opened delete confirmation",
            docId: deleteModal.docId,
          },
        ],
        url: "/documentation/mld",
      });
    ...
    (async () => {
      ...
      try {
        ...
        const results = await Promise.all(
          pairs.map(([src, tgt]) => fetchMappingPair(src, tgt)),
        );
        const map = {};
        pairs.forEach(([src, tgt], i) => {
          ...
          data.forEach((m) => {
            ...
            if (
              !map[k].find(
                (e) => e.code === m.targetControlCode && e.framework === tgt,
              )
            )
  ...
  const allDocRows = useMemo(() => {
    ...
    // console.log(
    //   "SoA map keys for 5.15:",
    //   Object.keys(soaMap).filter((k) => k.includes("5.15")),
    // );
    // console.log(
    //   "A.5.15 raw SoA entries:",
    //   soas.filter((s) => String(s.category).includes("5.15")),
    // );
    // console.log(
    //   "A.5.15 control from backend:",
    //   backendControls.find((c) => (c.controlCode || "").includes("5.15")),
    // );
    ...
    backendControls.forEach((ctrl) => {
      ...
      docsList.forEach(({ doc, type, dept }) => {
        ...
        const soaEntry =
          soaMap[`${framework}:${cat}:${docName}`] ||
          soaMap[`${framework}:${cat}:${type}`] ||
          soaMap[`${framework}:${cat}`] ||
          null;
        ...
        rows.push({
          ...
          docDept: dept || "",
          controlCode: cat,
          controlTitle: ctrl.title || ctrl.description || "",
          framework,
          soaEntry,
          controlMongoId: ctrl._id || ctrl.id || null,
        });
      });
    });
  ...
  const getLatestDocForSoA = useCallback(
    (soaId, docName, controlCode, docType) => {
      const allMatching = documents
        .filter((d) => String(d.soaId) === String(soaId))
        .sort((a, b) => b.version - a.version);

      if (allMatching.length === 0) return null;

      const nonDeleted = allMatching.filter((d) => !d.deleted);

      // Priority 1: composite key match — "A.5.15::POL" or "A.5.15::LOG"
      // This is set at upload time and is 100% filename-independent
      if (controlCode && docType) {
        const compositeKey = `${controlCode}::${docType}`.toLowerCase();
        const compositeMatch = nonDeleted.find(
          (d) => (d.controlId || "").toLowerCase() === compositeKey,
        );
        if (compositeMatch) return compositeMatch;
      }

      // Priority 2: composite with docName fallback (when docType is empty)
      if (controlCode && docName) {
        const compositeKey = `${controlCode}::${docName}`.toLowerCase();
        const compositeMatch = nonDeleted.find(
          (d) => (d.controlId || "").toLowerCase() === compositeKey,
        );
        if (compositeMatch) return compositeMatch;
      ...
      // Multiple docs, none matched — don't bleed
      return null;
    },
    [documents],
  );
  ...
  const filteredDocRows = useMemo(() => {
    ...
    if (!isAllSelected) {
      const mldFrameworks = selectedFrameworks
        .map((fw) => fwLabelToCode[fw])
        .filter(Boolean);
    ...
    if (uploadFilter !== "ALL") {
      list = list.filter((r) => {
        ...
        const doc = soaId
          ? getLatestDocForSoA(soaId, r.docName, r.controlCode, r.docType)
          : null;
    ...
    if (soaSearch.trim()) {
      ...
      list = list.filter(
        (r) =>
          ...
          (mappingsByControl[`${r.framework}:${r.controlCode}`] || []).some(
            (m) => m.code.toLowerCase().includes(q),
          ),
      );
    }
    ...
    if (colSort.key && soaSort === "none") {
      list.sort((a, b) => {
        ...
        switch (colSort.key) {
          case "cId":
            // ── NEW: sort by unified Control ID ──────────────────────────
          ...
          case "uploadDate": {
            const docA = getDoc(a);
            const docB = getDoc(b);
            valA = docA?.createdAt ? new Date(docA.createdAt).getTime() : 0;
            valB = docB?.createdAt ? new Date(docB.createdAt).getTime() : 0;
            break;
          }
          case "submissionDate": {
            const docA = getDoc(a);
            const docB = getDoc(b);
            valA = docA?.approvalDate
              ? new Date(docA.approvalDate).getTime()
              : 0;
            valB = docB?.approvalDate
              ? new Date(docB.approvalDate).getTime()
              : 0;
            break;
          }
          case "reviewDate": {
            const docA = getDoc(a);
            const docB = getDoc(b);
            valA = docA?.nextApprovalDate
              ? new Date(docA.nextApprovalDate).getTime()
              : 0;
            valB = docB?.nextApprovalDate
              ? new Date(docB.nextApprovalDate).getTime()
              : 0;
            break;
          }
          case "docName":
            valA = a.docName.toLowerCase();
            valB = b.docName.toLowerCase();
            break;
          case "controlCode":
            valA = frameworkSortKey(a.controlCode, a.framework);
            valB = frameworkSortKey(b.controlCode, b.framework);
            break;
          case "status": {
            const order = {
              approved: 0,
            ...
            const docA = getDoc(a);
            const docB = getDoc(b);
            valA = order[deriveStatus(a.soaEntry, docA)] ?? 9;
            valB = order[deriveStatus(b.soaEntry, docB)] ?? 9;
            break;
          }
          default:
            return 0;
    ...
    switch (soaSort) {
      ...
      case "date_newest":
        list.sort(
          (a, b) =>
            (b.soaEntry?.createdAt
              ? new Date(b.soaEntry.createdAt).getTime()
              : 0) -
            (a.soaEntry?.createdAt
              ? new Date(a.soaEntry.createdAt).getTime()
              : 0),
        );
        break;
      case "date_oldest":
  ...
  const frameworkCounts = useMemo(
    () =>
      Object.fromEntries(
        availableFrameworks.map((fw) => [
          fw.code,
          allDocRows.filter((r) => r.framework === fw.code).length,
        ]),
      ),
    [allDocRows, availableFrameworks],
  );

  const handlePreviewClick = (soaEntry, controlCode, docType, docName) => {
    const compositeKey = `${controlCode}::${docType || docName}`.toLowerCase();
    const doc =
      documents
        .filter((d) => String(d.soaId) === String(soaEntry.id) && !d.deleted)
        .sort((a, b) => b.version - a.version)
        .find((d) =>
          controlCode && (docType || docName)
            ? (d.controlId || "").toLowerCase() === compositeKey
            : true,
        ) ??
      documents.find(
        (d) => String(d.soaId) === String(soaEntry.id) && !d.deleted,
      );
    if (doc) {
      captureActivity({
        action: ACTIONS.CLICK,
        item:
          "Documentation · Previewed policy for control " +
          (soaEntry.clause || soaEntry.controlCode || soaEntry.id),
        url: "/documentation/mld",
      });
  ...
  const handleSingleButtonUpload = async (
    soaId,
    expectedDocName,
    docDept,
    controlCode,
    ...
    input.onchange = async (e) => {
      ...
      if (expectedDocName && file.name !== expectedDocName) {
        window.confirm(
          `Note: Your file "${file.name}" will be stored as "${expectedDocName}" to match this policy row.`,
        );
      }

      const resolvedDept =
        docDept ||
        user?.departments?.find((d) => d.name !== "N/A")?.name ||
      ...
      try {
        ...
        await documentationService.uploadDocument({
          file, // ← original file, no renaming
          soaId,
          controlId: `${controlCode}::${docType || expectedDocName}`,
  ...
  const sortAscLabel = () => {
    ...
    if (frameworkFilter === "KSA_PDPL")
      return "Control ID (Article-1 → Article-2 → …)";
    if (frameworkFilter === "ALL")
      return "Control ID (↑ Asc, grouped by framework)";
    return "Control ID (4.x…10.x → A.x…)";
  };
  const sortDescLabel = () => {
    if (frameworkFilter === "SOC2") return "Control ID (P → PI → C → A → CC)";
    if (frameworkFilter === "KSA_PDPL")
      return "Control ID (… → Article-2 → Article-1)";
    if (frameworkFilter === "ALL")
      return "Control ID (↓ Desc, grouped by framework)";
  ...
  return (
    <div style={{ padding: "10px", maxWidth: "1400px", margin: "0px auto" }}>
      ...
      {[
        {
          label: "Tutorial",
          bg: "linear-gradient(90deg,#ffb74d,#ff9800)",
      ...
      >
        ...
        {controlsLoading ? (
          ...
          >
            ...
            <div>
              <span style={{ fontWeight: 600 }}>Uploaded:</span> {docCount}
            </div>
            <div>
              <span style={{ fontWeight: 600 }}>SoA Linked:</span>{" "}
              {allDocRows.filter((r) => r.soaEntry).length}
            </div>
            {availableFrameworks.map((fw) => (
              <div key={fw.code}>
                <span style={{ fontWeight: 600 }}>{fw.label}:</span>{" "}
                {frameworkCounts[fw.code] || 0}
              </div>
            ))}
          </div>
        )}
        ...
        >
          ...
          >
            <option value="framework">
              By Framework (27001 → 27701 → SOC2 → 42001 → PDPL)
            </option>
            ...
            {soaSort === "none" && (
              <option value="none">Column Sort Active</option>
            )}
          </select>

          <div
            id="mld-upload-filter"
          ...
          >
            ...
            {[
              ...
              <button
                key={val}
                style={filterPill(uploadFilter === val)}
          ...
          <table id="mld-upload-table" className="mld-table">
            <thead>
              <tr style={{ backgroundColor: "#f8f9fa" }}>
                {/* ── COLUMN 1: Control ID (moved first) ── */}
                <SortableHeader
                  label="Control ID"
                  sortKey="cId"
                  currentSort={colSort}
              ...
              {controlsLoading ? (
                ...
                filteredDocRows.map(
                  (
                    ...
                    const doc = soaId
                      ? getLatestDocForSoA(soaId, docName, controlCode, docType)
                      : null;
                    const isUploaded = !!doc && !doc.deleted;
                    const isSoaLinked = !!soaEntry;
                    const mappings =
                      mappingsByControl[`${framework}:${controlCode}`] || [];
                    const rowBg = isSoaLinked ? "#f0fff4" : "#ffffff";

                    const status = deriveStatus(
                      soaEntry,
                      doc && !doc.deleted ? doc : null,
                    );
                    ...
                    const ownership =
                      ownershipMap[
                        `${framework.trim()}:${controlCode.trim()}`
                      ] || {};
                    const approvedBy =
                      doc?.approvedBy ||
                      doc?.approverName ||
                    ...
                    const handleApprove = async () => {
                      ...
                      try {
                        ...
                        captureActivity({
                          ...
                          item:
                            "Documentation · Approved policy '" +
                            (doc.docName || doc.id) +
                            "' for control " +
                            (soaEntry?.clause || soaEntry?.controlCode || ""),
                          url: "/documentation/mld",
                        });
                        setModal({
                          isOpen: true,
                    ...
                    return (
                      <tr
                        ...
                        style={{
                          ...
                          borderLeft: isSoaLinked
                            ? "4px solid #28a745"
                            : "4px solid transparent",
                          transition: "background-color 0.15s ease",
                        }}
                        onMouseEnter={(e) => {
                          e.currentTarget.style.backgroundColor = isSoaLinked
                            ? "#e6f9ed"
                            : "#f8f9fa";
                        }}
                        onMouseLeave={(e) => {
                      ...
                      >
                        {/* ── COLUMN 1: Control ID — unified ID only ── */}
                        <td
                          style={{
                            padding: "12px 14px",
                        ...
                        <td
                          ...
                          style={{
                            ...
                            verticalAlign: "middle",
                            cursor:
                              isUploaded && soaEntry ? "pointer" : "default",
                            color: doc?.deleted ? "#999" : "#2c3e50",
                          }}
                        >
                          {docName}
                          {isSoaLinked && (
                        ...
                        {/* Control Code + requirement title */}
                        <td
                          style={{
                            padding: "12px 14px",
                            verticalAlign: "middle",
                            ...
                            {soaId ? (
                              <button
                                ...
                                style={{
                                  ...
                                  alignItems: "center",
                                  justifyContent: "center",
                                  color: isUploaded ? "white" : "inherit",
                                  minWidth: "80px",
                                }}
                              >
                                {uploading[soaId] ? (
                                ...
                                <button
                                  onClick={async () => {
                                    captureActivity({
                                      action: ACTIONS.CLICK,
                                      item:
                                        "Documentation · Checked version history for document " +
                                        doc.id,
                                      url: "/documentation/mld",
                                    });
                                    const versions =
                                      await documentationService.getDocVersions(
                                        doc.id,
                                      );
      ...
      <Modal
        ...
        onConfirm={async () => {
          try {
            await documentationService.deleteDocument(
              deleteModal.docId,
              deleteModal.comment,
            );
            captureActivity({
              ...
              item:
                "Documentation · Archived policy " +
                deleteModal.docId +
                (deleteModal.comment
                  ? " — Reason: " + deleteModal.comment
                  : ""),
              url: "/documentation/mld",
            });
            setDocuments((prev) =>
              prev.map((d) =>
                d.id === deleteModal.docId
                  ? {
                      ...d,
                      deleteComment: deleteModal.comment,
                      deleted: true,
            ...
            setModal({
              ...
              title: "Archived",
              message:
                "Policy moved to Archive. You can permanently delete it from the Archived section in the dashboard.",
              showCancel: false,
              onConfirm: () => setModal((m) => ({ ...m, isOpen: false })),
            });
          } catch (e) {
        ...
        message={
          <div>
            ...
            >
              This policy will be moved to <strong>Archive</strong>. You can
              permanently delete it from the dashboard's Archived section.
            </p>
            <textarea
              placeholder="Enter reason for archiving (Remarks)"
              value={deleteModal.comment}
              onChange={(e) =>
                setDeleteModal((m) => ({ ...m, comment: e.target.value }))
              }
              style={{
                width: "100%",
                minHeight: 80,
      ...
      {addTaskModal.open && (
        <AddTaskModal
          ...
          onSuccess={() => {
            const name =
              addTaskModal.row?.docName || addTaskModal.row?.cId || "control";
            setTaskSuccessMsg(
              `Task created for "${name}" — visible in Task Management & My Tasks.`,
            );
