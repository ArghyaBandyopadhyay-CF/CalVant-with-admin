class riskService {
  constructor() {
    ...
    this.username = `${process.env.REACT_APP_BACKEND_USER_NAME}`;
    this.password = `${process.env.REACT_APP_BACKEND_USER_PWD}`;
  }
  ...
  // --- Delete one risk
  async deleteRisk(riskId, organization) {
    try {
      const response = await fetch(
        `${this.baseUrl}/${riskId}?org=${organization}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
