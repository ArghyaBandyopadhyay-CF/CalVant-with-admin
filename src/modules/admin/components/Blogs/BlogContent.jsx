...
export default function BlogContent() {
  ...
  const API_BASE = `${process.env.REACT_APP_SP}/blog-service/api/blogs`;

  const fetchBlogs = async () => {
    try {
      setLoading(true);
      const res = await fetch(`${API_BASE}?page=${page + 1}&limit=${rowsPerPage}`, { headers: getAdminFetchHeaders() });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/${id}`, {headers: getAdminFetchHeaders({"Content-Type": "application/json"}),  method: "DELETE" });
      if (res.ok) {
    ...
    <Box sx={{ p: 4, width: '100%', maxWidth: '1200px', margin: '0 auto', fontFamily: "'Inter', sans-serif" }}>
      ...
      <Box sx={{
        ...
        <Button
          ...
          onClick={() => history.push('/blogs/content/add')}
          sx={{
            borderRadius: '24px',
            textTransform: 'none',
            fontWeight: 700,
                ...
                >
                  ...
                  <TableCell align="right">
                    <Tooltip title="Edit Post" TransitionComponent={Fade}>
                      <IconButton
                        onClick={() => history.push(`/blogs/content/edit/${blog.slugUrl || blog.slug || blog.id}`)}
