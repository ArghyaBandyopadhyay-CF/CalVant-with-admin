...
export default function FooterContentEdit() {
  ...
  useEffect(() => {
    const fetchContent = async () => {
      try {
        setLoading(true);
        const res = await fetch(`${API_BASE}/type/${effectiveType}`, { headers: getAdminFetchHeaders() });
  ...
  const handleSaveSubmit = async () => {
    ...
    try {
      ...
      history.push("/footer-content/list");
    } catch (err) {
      console.error(err);
      alert("Update failed");
    } finally {
  ...
  return (
    <Box sx={{ p: 4, maxWidth: 1200, margin: "auto" }}>
      ...
      <Box sx={{ display: "flex", justifyContent: "space-between", mb: 4 }}>
        <Button startIcon={<ArrowBackIcon />} onClick={() => history.push("/footer-content/list")}>
          Back to List
        </Button>
