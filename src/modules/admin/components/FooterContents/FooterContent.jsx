...
export default function FooterContent() {
  ...
  const API_BASE = `${process.env.REACT_APP_SP}/footer-service/api/footer-content`;

  const fetchContents = async () => {
    try {
      setLoading(true);
      const res = await fetch(`${API_BASE}/list`, { headers: getAdminFetchHeaders() });
    ...
    <Box sx={{ p: 4, width: '100%', maxWidth: '1200px', margin: '0 auto', fontFamily: "'Inter', sans-serif" }}>
      ...
      <Box sx={{ 
        ...
        <Button 
          ...
          onClick={() => history.push('/footer-content/add')}
          sx={{ 
            borderRadius: '24px', 
            textTransform: 'none', 
            fontWeight: 700, 
                ...
                >
                  ...
                  <TableCell align="right">
                    <Tooltip title="Edit Content" TransitionComponent={Fade}>
                      <IconButton 
                        onClick={() => history.push(`/footer-content/edit/${content.type}`)}
