...
// const LOGGING_BASE_URL = process.env.REACT_APP_SP + "/logging-service/api/logs";
...
// ── Auth helper ───────────────────────────────────────────────────────────────
// Sends Authorization, x-org, x-region, AND x-role so the backend can
// correctly scope logs (super_admin sees all; org users see only their org).
// const getAdminFetchHeaders = (extra = {}) => {
//     const token = sessionStorage.getItem("token");
//     const user  = JSON.parse(sessionStorage.getItem("user") || "{}");
//     return {
//         Authorization : token             ? `Bearer ${token}` : undefined,
//         "x-org"       : user.organization || undefined,
//         "x-region"    : user.region       || undefined,
//         "x-role"      : user.role         || undefined,   // ← drives backend scoping
//         ...extra,
//     };
// }
...
//     // role is an array e.g. ['root'] or ['super_admin'] — extract first element
//     const role = Array.isArray(user.role) ? user.role[0] : (user.role || undefined);

//     // organization is an ID — we need the name from somewhere else
//     // For now send the ID, we'll fix the backend to handle this
//     const org = user.organizationName || user.organization || undefined;
...
const getAdminFetchHeaders = (extra = {}) => {
    ...
    // Extract role from array if needed
    const role = Array.isArray(user.role) ? user.role[0] : (user.role || undefined);

    // Region with no hardcoded fallback
    const region = user.region || sessionStorage.getItem("selected_region");

    return {
        Authorization : token             ? `Bearer ${token}` : undefined,
        "x-org"       : user.organization || undefined,
        "x-role"      : role              || undefined,
        "x-region"    : (region && region !== "AUTO") ? region : undefined,
        ...extra,
    };
};
...
const LOGGING_BASE_URL = process.env.REACT_APP_SP + "/logging-service/api/logs";
//const LOGGING_BASE_URL = process.env.REACT_APP_LS + "/api/logs";
// ── Action chip colours ───────────────────────────────────────────────────────
const ACTION_META = {
    PAGE_LOAD : { color: "primary", label: "PAGE LOAD" },
    LOGIN     : { color: "success", label: "LOGIN"     },
...
const getChipProps = (action = "") =>
    ACTION_META[action] ?? { color: "default", label: action };

// ── Helpers ───────────────────────────────────────────────────────────────────

/**
 * Java's LocalDateTime serialises as "2025-05-20T14:32:00" — no trailing Z.
 * Browsers parse that as *local* time, which is wrong when the server runs
 * in UTC on AWS. Appending "Z" forces UTC; toLocaleString("en-IN") then
 * converts to IST automatically.
 */
const toDateStr = (log) => {
    const raw = log.timestamp || log.createdAt;
    if (!raw) return null;
...
const ListOfLogs = () => {
    ...
    const fetchLogs = useCallback(async () => {
        ...
        try {
            const res = await fetch(LOGGING_BASE_URL, {
                // x-role drives super_admin vs org scoping on the backend
                // x-org  scopes org-level users to their own org only
