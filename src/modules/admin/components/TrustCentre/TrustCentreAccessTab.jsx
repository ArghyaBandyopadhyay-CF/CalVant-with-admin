...
import {
  CheckCircle, Cancel, ContentCopy, Link, Refresh,
  PersonAdd, Email, VerifiedUser, LinkOff, OpenInNew,
  Policy, Shield,
} from "@mui/icons-material";
import {
  toggleShare,
...
// ─────────────────────────────────────────────────────────────────────────────
// Props:
//   tc          — the TrustCentre object from parent state
//   setTc       — state setter
//   isPublished — boolean
// ─────────────────────────────────────────────────────────────────────────────
export default function TrustCentreAccessTab({ tc, setTc, isPublished }) {
  ...
  const handleToggleShare = async (enabled) => {
    ...
    setToggling(true);
    try {
      const result = await toggleShare(enabled);
      setTc((prev) => ({
        ...prev,
        shareEnabled: result.shareEnabled,
        shareToken:   result.shareToken || prev.shareToken,
            ...
            <Box sx={{ px: 3, py: 2.5 }}>
              ...
              <Box sx={{
                ...
                <Box sx={{ display: "flex", gap: 0.5, flexShrink: 0 }}>
                  ...
                  <Tooltip title="Open in new tab">
                    <IconButton size="small" onClick={() => window.open(shareUrl, "_blank")}
                      sx={{ color: ACCENT, "&:hover": { bgcolor: "#dceeff" } }}>
                      <OpenInNew fontSize="small" />
                    </IconButton>
                  </Tooltip>
                ...
                {pendingRequests.map((req) => (
                  <TableRow key={req.id} hover>
                    ...
                    <TableCell>
                      {req.targetPolicyName ? (
                        <Chip
                          label={req.targetPolicyName} size="small"
                          icon={<Policy style={{ fontSize: 12 }} />}
                          sx={{ bgcolor: "#faf5ff", color: "#7c3aed", border: "1px solid #ede9fe", fontWeight: 600, fontSize: 11 }}
                        />
                      ) : (
                        <Chip
                          label="Full Access" size="small"
                          icon={<Shield style={{ fontSize: 12 }} />}
                        ...
                        <Tooltip title="Approve">
                          <span>
                            <IconButton
                              ...
                              onClick={() => handleApprove(req.id)}
                              disabled={actionId === req.id}
                              sx={{ color: SUCCESS, border: `1px solid ${SUCCESS}`, borderRadius: 1, p: 0.5, "&:hover": { bgcolor: "#d9f2e6" } }}
                            >
                              {actionId === req.id
                                ? <CircularProgress size={14} color="inherit" />
                                : <CheckCircle fontSize="small" />}
                        ...
                        <Tooltip title="Reject">
                          <span>
                            <IconButton
                              ...
                              onClick={() => handleReject(req.id)}
                              disabled={actionId === req.id}
                              sx={{ color: DANGER, border: `1px solid ${DANGER}`, borderRadius: 1, p: 0.5, "&:hover": { bgcolor: "#fff5f5" } }}
                            >
                              {actionId === req.id
                                ? <CircularProgress size={14} color="inherit" />
                                : <Cancel fontSize="small" />}
