...
//       const token = sessionStorage.getItem("token");
//       await axios.post(
//         `${process.env.REACT_APP_SP}/tprm-service/api/tprm/vendors`,
//         payload,
//         {
//           headers: {
...
function VendorCreate() {
  ...
  // Read from sessionStorage (admin panel uses sessionStorage, not localStorage)
  const token = sessionStorage.getItem("token");
  const decoded = token ? jwtDecode(token) : null;
  const loggedInRole = Array.isArray(decoded?.role)
    ? decoded.role[0]
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.vendorName.trim()) return setError("Vendor Name is required.");
    if (!formData.pocEmail.trim()) return setError("Contact Person Email is required.");
    if (!formData.pocName.trim()) return setError("Contact Person Name is required.");

    setLoading(true);

    try {
      const myObject = JSON.parse(sessionStorage.getItem("user") || "{}");
      const organizationId = myObject?.organization || decoded?.organization || null;
      const poc = formData.pocName.trim();

      // ── Step 1: Create Vendor ──────────────────────────────────────────────
      // Use hardcoded URL + explicit Authorization header,
      // exactly like the old working code — no env variable, no interceptor.
      const payload = {
        vendorName: formData.vendorName.trim(),
        organization: organizationId,
        address: formData.address.trim(),
          ...
          <form onSubmit={handleSubmit}>
            <Stack spacing={2}>
              ...
              <Box sx={{ pt: 1 }}>
                ...
                >
                  {loading ? "Registering..." : "Finalize & Record Vendor Profile"}
                </Button>
              </Box>
            </Stack>
          </form>
