  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", display: "flex", justifyContent: "center", alignItems: "center", zIndex: 10002, padding: 20, backdropFilter: "blur(4px)" }}>
      <div style={{ background: "#fff", borderRadius: 14, width: "100%", maxWidth: 420, boxShadow: "0 20px 60px rgba(0,0,0,0.25)", overflow: "hidden" }}>
        {/* Header — matches MyTasks blue theme */}
...
export default function TaskDetailPanel({ task, onClose, onUpdate, users = [], departments = [], currentUser }) {
  ...
  // ── Subtask table column widths — fixed, no overlap ──────
  // Work(description) | Priority | Assignee | Due Date | Status | Del
  ...
  return (
    <>
      ...
      {/* Panel — white theme matching MyTasks */}
      <div ref={panelRef} style={{
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 20px", borderBottom: "1px solid #f1f5f9", flexShrink: 0, background: "#fff" }}>
          ...
          <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer", color: "#94a3b8", fontSize: 18, padding: "4px 8px", borderRadius: 6, lineHeight: 1, transition: "color 0.15s, background 0.15s" }}
              ...
              {subTasksOpen && (
                <div style={{ border: "1px solid #e2e8f0", borderRadius: 10, overflow: "hidden" }}>
                  ...
                  {subTasksLoading ? (
                    ...
                    // ── FIX: endDate is always the due date; never show it in Work col ──
                    ...
                    return (
                      ...
                      >
                        {/* ── Work col: ONLY description, no date leaking in ── */}
                        ...
                        {/* ── Due Date col: ALWAYS shows date, never bleeds into Work ── */}
                        <div style={{ overflow: "hidden" }}>
                          {stEndDate
                  ...
                  {/* Add new subtask row — same grid, each cell directly editable */}
          ...
          <div style={{ width: 240, flexShrink: 0, overflowY: "auto", padding: "24px 18px", background: "#fafbfc", borderLeft: "1px solid #f1f5f9" }}>
            ...
            {task?.riskId && (
              <div style={{ marginBottom: 18 }}>
                ...
                <span style={{ fontSize: 12, fontWeight: 700, color: "#1971c2", background: "#e7f5ff", padding: "2px 8px", borderRadius: 4 }}>{task.riskId}</span>
              </div>
            )}
            {task?.auditId && (
              <div style={{ marginBottom: 18 }}>
                <label style={sidebarLabel}>Audit ID</label>
                <span style={{ fontSize: 12, fontWeight: 700, color: "#e67700", background: "#fff9db", padding: "2px 8px", borderRadius: 4 }}>{task.auditId}</span>
              </div>
            )}
